...
def _decision_from_eval(
    ...
    if not sat:
        ...
        return "BLOCK", "sat_false", failed
    if gained_resource_ids:
        if ceval_constraints_total:
            return "ALLOW", "totally_calculable_constraint_gain", failed
        if ceval_constraints_partial:
            return "ALLOW", "partially_calculable_constraint_gain", failed
        return "ALLOW", "resource_gain", failed
    if failed:
        return "BLOCK", f"failed_{failed[0]}", failed
    return "BLOCK", "no_marginal_contribution", failed
...
def evaluate_with_objective_and_session(
    ...
    decision, decision_reason, failed_predicates = _decision_from_eval(sat, ceval_constraints_total, ceval_constraints_partial, gained_resource_ids, clauses)
