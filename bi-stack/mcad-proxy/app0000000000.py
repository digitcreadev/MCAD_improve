...
async def mcad_session_new(req: Request):
    ...
    session = _relay_post("/sessions", {"objective_id": objective_id, "dw_id": dw_id})
...
async def mcad_session_resume(req: Request):
    ...
    sessions = _relay_get("/sessions")
    session = next((s for s in sessions if str(s.get("session_id")) == session_id), None)
    if session is None:
...
def mcad_history_current():
    ...
    if not sid:
        return {"ok": True, "session_id": None, "items": []}
    timeline = _relay_get(f"/analytics/sessions/{sid}/timeline")
    items = []
    for step in timeline.get("steps", []):
        allow = bool(step.get("sat")) and float(step.get("phi") or 0.0) > 0.0
        items.append({
            "step_index": step.get("t"),
            "decision": "ALLOW" if allow else "BLOCK",
            "decision_reason_code": "SAT_OK_CONTRIBUTIVE" if allow else "SAT_FALSE_OR_NO_GAIN",
            "decision_reason": "Timeline-derived summary",
            "phi": step.get("phi"),
            "delta_phi_t": step.get("delta_phi_t"),
            "newly_contributed_constraints_total": step.get("calculable_constraints") or [],
            "newly_contributed_constraints_partial": [],
            "gained_resource_ids": [],
            "mdx": "",
        })
    return {"ok": True, "session_id": sid, "objective_id": oid, "items": items}
...
def mcad_graph_current():
    ...
    oid = ACTIVE_CONTEXT.get("objective_id")
    if not oid:
        return {"ok": True, "session_id": sid, "graph": {"nodes": [], "edges": []}, "metrics": {}}
    sessions_data = _relay_get("/sessions")
    sessions = sessions_data if isinstance(sessions_data, list) else sessions_data.get("items", [])
    state = next((s for s in sessions if str(s.get("session_id")) == str(sid)), None) if sid else None
    objective = _relay_get(f"/objectives/{oid}")
    constraints = objective.get("constraints", []) or []
    covered = set((state or {}).get("covered_constraints") or [])
    timeline = _relay_get(f"/analytics/sessions/{sid}/timeline") if sid else {"steps": []}
    partial = set()
    for st in timeline.get("steps", []):
        for cid in st.get("calculable_constraints", []) or []:
            partial.add(cid)
    nodes = [{"id": oid, "label": objective.get("name") or oid, "kind": "objective", "status": "objective"}]
    edges = []
    for c in constraints:
        cid = c.get("id") if isinstance(c, dict) else getattr(c, "id", None)
        if not cid:
            continue
        status = "total" if cid in covered else ("partial" if cid in partial else "none")
        nodes.append({"id": cid, "label": cid, "kind": "constraint", "status": status})
        edges.append({"source": oid, "target": cid})
    allow_count = sum(1 for st in timeline.get("steps", []) if bool(st.get("sat")) and float(st.get("phi") or 0.0) > 0.0)
    block_count = len(timeline.get("steps", [])) - allow_count
    total = max(1, len(timeline.get("steps", [])))
    metrics = {
        "completion_rate": len(covered) / max(1, len(constraints)),
        "calculability_rate_total": len(covered) / max(1, len(constraints)),
        "calculability_rate_partial": len(partial) / max(1, len(constraints)),
        "analytic_alignment_score": (allow_count - block_count) / total,
        "allow_rate": allow_count / total,
        "allow_count": allow_count,
        "block_count": block_count,
    }
    return {"ok": True, "session_id": sid, "objective_id": oid, "graph": {"nodes": nodes, "edges": edges}, "metrics": metrics}
