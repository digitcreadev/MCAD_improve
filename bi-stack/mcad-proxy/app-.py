...
def _relay_post_first(paths: list[str], payload: dict) -> dict:
    last_err = None
    for path in paths:
        try:
            return _relay_post(path, payload)
    ...
    if last_err:
        raise last_err
    raise RuntimeError("No relay path available")


def _unwrap_items(data: dict | list | None) -> list:
...
def _unwrap_session(data: dict | list | None) -> dict:
...
def _normalize_history_item(item: dict) -> dict:
    return {
        "step_index": item.get("step_index") or item.get("t"),
        "decision": str(item.get("decision") or "").upper(),
        "decision_reason_code": item.get("decision_reason_code") or item.get("reason_code") or "",
        "decision_reason": item.get("decision_reason") or item.get("reason") or item.get("explain") or "",
        "phi": item.get("phi", 0.0),
        "delta_phi_t": item.get("delta_phi_t", 0.0),
        "newly_contributed_constraints_total": item.get("newly_contributed_constraints_total") or [],
        "newly_contributed_constraints_partial": item.get("newly_contributed_constraints_partial") or [],
        "gained_resource_ids": item.get("gained_resource_ids") or [],
        "mdx": item.get("mdx") or "",
        "query_digest": item.get("query_digest") or "",
...
def _get_session_history(session_id: str | None) -> list[dict]:
    if not session_id:
        return []
    data = _relay_get(f"/sessions/{session_id}/history")
    return [_normalize_history_item(it) for it in _unwrap_items(data)]
...
def mcad_session_current():
    ...
    last = dict(LAST_DECISION)
    try:
        hist = _get_session_history(sid)
        if hist:
            last = hist[-1]
    except Exception:
        pass
    return {"ok": True, "active": ACTIVE_CONTEXT, "last_decision": last}
...
async def mcad_session_new(req: Request):
    ...
    session_payload = _relay_post_first(["/sessions/create", "/sessions"], {"objective_id": objective_id, "dw_id": dw_id})
    session = _unwrap_session(session_payload)
...
async def mcad_session_resume(req: Request):
    ...
    sessions = _unwrap_items(_relay_get("/sessions"))
...
def mcad_history_current():
    ...
    items = _get_session_history(sid)
...
def mcad_graph_current():
    ...
    oid = ACTIVE_CONTEXT.get("objective_id")
    if not sid:
        return {"ok": True, "session_id": sid, "objective_id": oid, "graph": {"nodes": [], "edges": []}, "metrics": {}}
    ...
    return {
        "ok": True,
        "session_id": sid,
        "objective_id": data.get("objective_id") if isinstance(data, dict) else oid,
        "dw_id": data.get("dw_id") if isinstance(data, dict) else ACTIVE_CONTEXT.get("dw_id"),
